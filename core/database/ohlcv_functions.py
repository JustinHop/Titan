...
def insert_data_into_ohlcv_table(exchange, pair, interval, candle, pair_id):
    ...
    with database.lock:
        args = [
            ...
            candle[5],
            interval]
        ins = database.OHLCV.insert().values(
            Exchange=args[0],
            Pair=args[1],
...
def get_latest_candle(exchange, pair, interval):
    ...
    with database.lock:
        ...
        s = select(
            [database.OHLCV]).where(
            and_(
                database.OHLCV.c.Exchange == exchange, database.OHLCV.c.Pair ==
                pair, database.OHLCV.c.Interval == interval)).order_by(
            database.OHLCV.c.TimestampRaw.desc()).limit(1)
    ...
    with database.lock:
        s = select(
            ...
            and_(
                database.OHLCV.c.Exchange == exchange, database.OHLCV.c.Pair ==
                pair, database.OHLCV.c.Interval == interval)).order_by(
            database.OHLCV.c.ID.desc()).limit(N)
        result = conn.execute(s)
...
def convert_timestamp_to_date(timestamp):
    # this might only work on bittrex candle timestamps

...
class MarketSimulator(Market):
    ...
    def __init__(self, exchange, base_currency, quote_currency,
                 quote_currency_balance, strategy):
    ...
    def limit_buy(self, quantity, price):
        if self.quote_balance >= quantity * price:
            ...
            logger.info(
                ...
                " " +
                self.quote_currency)
            logger.info(self.quote_currency + " balance: " +
                        str(self.quote_balance))
            logger.info(self.base_currency + " balance: " +
                        str(self.base_balance))
        else:
    ...
    def market_buy(self, quantity):
        if self.quote_balance >= quantity * self.get_ask_price():
            ...
            self.base_balance = self.base_balance + quantity
            logger.info("Executed buy simulation of " +
                        str(quantity) +
                        " " +
                        self.base_currency +
                        " for " +
                        str(self.get_ask_price()) +
                        " " +
                        self.quote_currency)
            logger.info(self.quote_currency + " balance: " +
                        str(self.quote_balance))
    ...
    def market_sell(self, quantity):
        if self.base_balance >= quantity:
            ...
            logger.info("Executed sell simulation of " +
                        str(quantity) +
                        " " +
                        self.base_currency +
                        " for " +
                        str(self.get_bid_price()) +
                        " " +
                        self.quote_currency)
            logger.info(self.quote_currency + " balance: " +
                        str(self.quote_balance))
...
def open_long_position_simulation(
        market, amount, price, fixed_stoploss, trailing_stoploss_percent,
        profit_target_percent):
    """Create simulated long position"""
    logger.info("Opening simulated long position")
    position = LongPositionSimulator(
        market,
        amount,
        price,
        fixed_stoploss,
...
class LongPositionSimulator(position.LongPosition):
    ...
    def __init__(self, market, amount, price, fixed_stoploss,
                 trailing_stoploss_percent, profit_target_percent):
        super().__init__(market, amount, price, fixed_stoploss,
                         trailing_stoploss_percent, profit_target_percent)
    ...
    def liquidate_position(self):
        ...
        open_short_position_simulation(
            self.market,
            self.amount,
            self.market.latest_candle['5m'][3])
        self.is_open = False

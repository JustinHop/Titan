...
class SmaCrossoverSignal(BaseSignalGenerator):
    ...
    def check_condition(self, new_candle):
        ...
        if (self.sma.value is not None) & (self.fma.value is not None) & (
                self.vol_change.value is not None):
            ...
            # if we already have a closing high saved, we need to check whether
            # were still crossed over, and if we need to open a trade
            if self.cached_high is not None:
                ...
                if not self.fma.value > self.sma.value:  # if we're no longer fma > sma, forget about saved high
                    ...
                    self.cached_high = None
                    return False
                # open a trade if the latest high is greater than the cached
                # high
                if new_candle[2] > self.cached_high:
                    self.strategy.print_message(
                        "Current high of " + str(new_candle[2]) +
                        " has exceeded cached high of " +
                        str(self.cached_high) + ", buy signal generated")
                    self.cached_high = None
                    return True
                else:
                    return False

            # if fma is not already above sma, and has now crossed, and volume
            # is up 5% from last period, send trade signal
            elif self.cached_high is None and\
                    self.fma.value > self.sma.value and\
                    self.vol_change.value > 5:
                self.strategy.print_message(
                    "FMA has crossed SMA, caching current high of " +
                    str(new_candle[2]))
